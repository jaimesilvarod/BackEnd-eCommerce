...
function fillLista(Url, divID, idItemSelected, addItemAll = false) {
  $.ajax({
    ...
    if (addItemAll)
      $("#" + divID).append("<option value=-1>Seleccione una opción</option>");

    for (var i = 0; i < data.length; i++) {
      var opc = "<option ";
      if (idItemSelected == data[i].id) opc = opc + " selected ";
      opc =
        opc + " value=" + data[i].id + ">" + data[i].descripcion + "</option>";
      $("#" + divID).append(opc);
    }
  });
}

function showListadoCategorias() {
  $("#tabla").DataTable({
    ajax: "categorias/getlistadoCB",
    rowId: "dt_rowid",
    columns: [
      { data: "nombre" },
      { data: "idMarca" },
    ...
    columnDefs: [
      {
        ...
        render: function (data, type, row, meta) {
          return (
            '<a href="javascript:openEditCategoriaModal(' +
            data +
            ')">' +
            '<i class="fa fa-list-alt fa-2x" alt="Editar Categoría" aria-hidden="true"></i></a>  ' +
            '<a href="javascript:dialogrem(' +
            data +
            ')">' +
