function fxValidaFrm() {
  if ($("#regDescripcion").val().trim() == "")
    return "Ingrese Descripción del Producto";
  if ($("#regDescripcion").val().trim().length < 3)
    return "La Descripción del Producto debe tener mínimo 3 caracteres";

  if ($("#txtImpuesto").val().trim() == "")
    return "Ingrese Valor del Impuesto del Producto";
...
function showErrorModalMsg(Titulo, Mensaje) {
  ...
  $("#modalSmsBody").html(
    '<i class="fa fa-exclamation-circle" style="color:red" aria-hidden="true"></i> <b>' +
      Mensaje +
      "</b>"
  );
  $("#modalSms").modal("show");
...
function showSuccessModalMsg(Titulo, Mensaje) {
  $("#modalSmsTitle").text(Titulo);
  $("#modalSmsBody").html(
    '<i class="fa fa-info-circle" style="color:blue" aria-hidden="true"></i> <b>' +
      Mensaje +
      "</b>"
  );
  $("#modalSms").modal("show");
...
function fxSaveProducto() {
  var datos = JSON.stringify({
    ...
    modelo: $("#regModelo").val(),
    nombre: $("#regNombreProducto").val(),
